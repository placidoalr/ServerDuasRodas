  ...
  CREATE TABLE IF NOT EXISTS `DRDB`.`TBOM` ( /* cadastro da OM */
    ...
    PRIMARY KEY (`ID`),
      FOREIGN KEY (`SETOR_ATRIB`) REFERENCES `DRDB`.`TBLOC_INST` (`ID`),
    ...
    CREATE TABLE IF NOT EXISTS `DRDB`.`TBOPER_WITH_OM`( /* OPERAÇÕES DENTRO DE UMA OM */
    `ID` INT NOT NULL AUTO_INCREMENT,
    `IDOM` INT NOT NULL,
    `IDOPER` INT NOT NULL,
    PRIMARY KEY (`ID`),
    FOREIGN KEY (`IDOM`) REFERENCES `DRDB`.`TBOM` (`ID`),
    FOREIGN KEY (`IDOPER`) REFERENCES `DRDB`.`TBOPERACAO` (`ID`)
    );
