...
export class OMAction extends Action {
    ...
    private selectEquipWOMROTA() {
        return 'SELECT u.NOME UNAME, TB_OM_DESC_ROTA.DESC DESC, TBEQUIP.NOME EQUIPNOME,TBEQUIP.LOCAL,TBEQUIP.EQUIP_SUP , TBEQUIP_WITH_TBOM.OPER,TBEQUIP_WITH_TBOM.OPER_REALIZADA, TBMATERIAL.DESC MAT_UTIL, TBMATERIAL.UN_MEDIDA ,TBEQUIP_WITH_TBOM.QTDE_MAT from TBEQUIP_WITH_TBOM INNER JOIN TBEQUIP ON TBEQUIP_WITH_TBOM.IDEQUIP = TBEQUIP.ID INNER JOIN TBMATERIAL ON TBMATERIAL.ID = TBEQUIP.MAT_UTIL INNER JOIN TB_OM_DESC_ROTA ON TB_OM_DESC_ROTA.IDEQUIP = TBEQUIP.ID INNER JOIN TBUSUARIO u ON u.ID = TB_OM_DESC_ROTA.IDMANUT  where TBEQUIP_WITH_TBOM.IDOM = \'' + this.req.body.idom + '\';';
    }
